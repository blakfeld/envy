...
mod tests {
    ...
    fn is_installed_true() {
        ...
        assert!(RabbitmqModule.is_installed(&pm, &Dependency::simple("rabbitmq")).unwrap());
    }
    ...
    fn install_propagates_pm_error() {
        ...
        assert!(RabbitmqModule.install(&pm, &Dependency::simple("rabbitmq")).is_err());
    }

    #[test]
    fn is_running_true() {
        let pm = crate::package_manager::MockPackageManager { service_running: true, ..Default::default() };
        assert!(RabbitmqModule.is_running(&pm, &Dependency::simple("rabbitmq")).unwrap());
    }
