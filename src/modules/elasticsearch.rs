...
mod tests {
    ...
    fn is_installed_true() {
        ...
        assert!(ElasticsearchModule.is_installed(&pm, &Dependency::simple("elasticsearch")).unwrap());
    }
    ...
    fn install_propagates_pm_error() {
        ...
        assert!(ElasticsearchModule.install(&pm, &Dependency::simple("elasticsearch")).is_err());
    }

    #[test]
    fn is_running_true() {
        let pm = crate::package_manager::MockPackageManager { service_running: true, ..Default::default() };
        assert!(ElasticsearchModule.is_running(&pm, &Dependency::simple("elasticsearch")).unwrap());
    }
    ...
    fn start_delegates_to_pm() {
        let pm = crate::package_manager::MockPackageManager::default();
        assert!(ElasticsearchModule.start(&pm, &Dependency::simple("elasticsearch")).is_ok());
    }

    #[test]
    fn start_propagates_pm_error() {
        let pm = crate::package_manager::MockPackageManager { start_service_fails: true, ..Default::default() };
        assert!(ElasticsearchModule.start(&pm, &Dependency::simple("elasticsearch")).is_err());
    }

    #[test]
    fn stop_delegates_to_pm() {
        let pm = crate::package_manager::MockPackageManager::default();
        assert!(ElasticsearchModule.stop(&pm, &Dependency::simple("elasticsearch")).is_ok());
    }

    #[test]
    fn stop_propagates_pm_error() {
        let pm = crate::package_manager::MockPackageManager { stop_service_fails: true, ..Default::default() };
        assert!(ElasticsearchModule.stop(&pm, &Dependency::simple("elasticsearch")).is_err());
    }
