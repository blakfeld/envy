...
impl Module for RustModule {
    ...
    fn install(&self, _pm: &dyn PackageManager, dep: &Dependency) -> Result<()> {
        if which("rustup").is_err() {
            #[cfg(test)]
            let installer_cmd = std::env::var("ENVY_TEST_RUSTUP_INSTALLER_SCRIPT").unwrap_or_else(
                |_| concat!(
...
mod tests {
    ...
    fn rustup_bin_returns_some_with_non_empty_home() {
        if std::env::var("HOME").map(|h| !h.is_empty()).unwrap_or(false) {
            ...
            assert!(bin.contains(".cargo/bin/rustup"), "Expected ~/.cargo/bin/rustup, got {bin}");
        }
    }
    ...
    fn rust_is_installed_true_when_cargo_on_path() {
        ...
        if which::which("cargo").is_err() { return; }
        let pm = crate::package_manager::MockPackageManager::default();
        let dep = Dependency::simple("rust");
    ...
    fn rust_resolved_version_is_some_when_rustc_installed() {
        if which::which("rustc").is_err() { return; }
        let pm = crate::package_manager::MockPackageManager::default();
        let dep = Dependency::simple("rust");
        let ver = RustModule.resolved_version(&pm, &dep).unwrap();
        assert!(ver.is_some(), "Expected Some version since rustc is on PATH");
        let v = ver.unwrap();
    ...
    fn rust_is_installed_true_when_rustup_on_path() {
        if which::which("rustup").is_err() { return; }
        let pm = crate::package_manager::MockPackageManager::default();
        let dep = Dependency::simple("rust");
        assert!(RustModule.is_installed(&pm, &dep).unwrap(),
    ...
    fn rust_is_installed_false_when_neither_on_path() {
        ...
        if which::which("rustup").is_ok() || which::which("cargo").is_ok() { return; }
        let pm = crate::package_manager::MockPackageManager::default();
        let dep = Dependency::simple("rust");
        assert!(!RustModule.is_installed(&pm, &dep).unwrap(),
    ...
    fn rust_resolved_version_is_none_when_rustc_absent() {
        if which::which("rustc").is_ok() { return; }
        if which::which("cargo").is_ok() { return; }
        let pm = crate::package_manager::MockPackageManager::default();
        let dep = Dependency::simple("rust");
        let ver = RustModule.resolved_version(&pm, &dep).unwrap();
        assert!(ver.is_none(), "Expected None version when rustc is not installed");
    }
    ...
    fn rust_install_fails_for_invalid_toolchain() {
        ...
        extra.insert("toolchain".into(), serde_yaml::Value::String("invalid-toolchain-envy-test-xyz-12345".into()));
        let dep = Dependency::with_extra("rust", extra);
        let result = RustModule.install(&pm, &dep);
        assert!(result.is_err(), "install must fail for an invalid toolchain name");
    }
    ...
    fn rust_install_rustup_script_failure_propagated() {
        ...
        if which::which("rustup").is_ok() { return; }
        unsafe { std::env::set_var("ENVY_TEST_RUSTUP_INSTALLER_SCRIPT", "exit 1"); }
        let pm = crate::package_manager::MockPackageManager::default();
        let dep = Dependency::simple("rust");
        let result = RustModule.install(&pm, &dep);
        unsafe { std::env::remove_var("ENVY_TEST_RUSTUP_INSTALLER_SCRIPT"); }
        assert!(result.is_err(), "install must fail when rustup installer exits non-zero");
    }
    ...
    fn rust_install_rustup_script_success_does_not_bail() {
        ...
        if which::which("rustup").is_ok() { return; }
        unsafe { std::env::set_var("ENVY_TEST_RUSTUP_INSTALLER_SCRIPT", "exit 0"); }
        let pm = crate::package_manager::MockPackageManager::default();
        let dep = Dependency::simple("rust");
        let result = RustModule.install(&pm, &dep);
        unsafe { std::env::remove_var("ENVY_TEST_RUSTUP_INSTALLER_SCRIPT"); }
    ...
    fn rust_resolved_version_contains_dot_when_rustc_installed() {
        ...
        let ver = RustModule.resolved_version(&pm, &dep).unwrap();
        if let Some(v) = ver {
            assert!(v.contains('.'), "Expected semver-like version like '1.78.0', got: {v}");
